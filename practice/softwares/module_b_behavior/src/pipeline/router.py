...
from src.api.schemas import MediaType
...
def get_pipeline(scene: str = "classroom", label_mode: str | None = None):
    if scene == "extracurricular":
        from src.extracurricular.pipeline import ExtracurricularPipeline
        return ExtracurricularPipeline(label_mode=label_mode)
    from src.classroom.pipeline import ClassroomPipeline
